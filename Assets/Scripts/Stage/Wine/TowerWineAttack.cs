...
public class TowerWineAttack : MonoBehaviour
{
[SerializeField]
    ...
    private IEnumerator Attack(){
        while(true){
            ...
            for(int i=0;i<EnemyList.Count;i++){
                if(EnemyList[i].GetComponent<EnemyHP>().CurrentHP > maxhp){
                    maxhp = EnemyList[i].GetComponent<EnemyHP>().CurrentHP;
            ...
            if(AttackTarget != null){
                ...
                _bullet.GetComponent<BulletWineMove>().TargetSet(AttackTarget.transform);
                _bullet.GetComponent<BulletWineMove>().BulletSetUp(gameObject.transform.parent.gameObject);
            }
