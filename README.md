...
### 2. 데이터
> * 유저 데이터와 캐릭터 목록을 담는 클래스들은 그 목적과 사용이 다른 스크립트와 다르기 때문에 구별하여 다른 폴더에 저장하였습니다.   
> * 데이터 구조의 변화가 있을 경우 쉽게 수정할 수 있도록 했습니다.   
### 3. 프로젝트
...
## 🎮수정 진행중
2021년 10월과 2022년 5월에 진행되어 데모를 완성시켰으나 2022년 9월 코드 리뷰를 통해 부족한 점을 많이 발견했습니다.   
특히 하드코딩을 하여 코드가 많이 지저분하기 때문에 추가적인 공부를 통해 코드를 다듬는 과정을 진행 중입니다.
아래는 개선해야하는 내용과 계획으로 현재 진행형으로 수정이 진행 중입니다.   
#### 📌 최적화
* 클래스 상속 관계 정립   
...
> 1. 혹여나 존재하는 Update()에서의 GetComponent(), Find() 사용을 줄이고 가능하면 Start(), Awake() 등으로 이동시킵니다.   
> 2. 캐싱   
...
> 빈번한 생성과 파괴를 줄이기 위해 미리 임시 SD와 총알들을 생성하여 활성화/비활성화하는 방식을 사용하도록 합니다.
* List 자료구조 바꾸기   
